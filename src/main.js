...
let branch_validation_prefix = null; // for AWS: /^[a-z0-9-]{4,37}$/;
...
const getYargs = () => {
  if (build_vars_file && !build_vars_added) {
    ...
    module.exports.validateExistingVars(
      build_vars,
      yargv.env,
      yargv.branch || module.exports.getGitBranch()
    );
    Log.warn(
      "using EXISTING vars from build/vars.json: " + JSON.stringify(build_vars)
    );
...
module.exports.calcPathBuildVars = (build_vars) => {
  if (build_vars.origin === "set-in-build") {
    build_vars.origin =
      (build_vars.protocol || "https") + "://" + build_vars.domain;
    if (build_vars.port) {
  ...
  if (build_vars.path_prefix === "set-in-build") {
    ...
    if (
      (build_vars.env === "prod" || build_vars.env === "nonprod") &&
      build_vars.branch &&
      build_vars.branch !== "master"
    ) {
  ...
  if (build_vars.base_url === "set-in-build") {
    build_vars.base_url =
      build_vars.origin + (build_vars.path_prefix || "") + "/";
  }
...
module.exports.convertSourceToTarget = (
  source_prefix,
  target_prefix,
  from_suffix,
  to_suffix
...
module.exports.createBuildVarsFile = (build_vars_source_file) => {
  if (!build_vars_file) {
    throw new Error(
      `setBuildVarsFile() not called - required to define the Build Vars file location`
    );
  }
  if (typeof build_vars_source_file !== "string" || !build_vars_source_file) {
    throw new Error(
      `setBuildVarsFile() source file must be a non-blank string`
    );
  }
  if (build_vars_added) {
    throw new Error(`getArgs() already called`);
  }
  const build_vars_orig = module.exports.getJSONFileData(
    build_vars_source_file
  );
  const build_vars_new = module.exports.calcBuildVarsFromEnv(
    build_vars_orig,
    yargv.env
  );
...
module.exports.getBuildFunctions = function () {
  ...
  out.run = async () => {
    if (yargv.t) {
      ...
      taskset.run(target).catch((error) => {
...
module.exports.getRelativePath = (abs_path) => {
  if (abs_path.indexOf(process.cwd()) !== 0) {
    throw new Error(
      `${abs_path} doesn't seem to be an absolute path, based on cwd ${process.cwd()}`
    );
  }
...
module.exports.gitIsClean = () => {
  ...
  return (
    out &&
    out.reduce(
      (prev, line) => prev || line === "nothing to commit, working tree clean",
      false
    )
  );
};
...
module.exports.validateExistingVars = (data, env, branch) => {
  if (data.env !== env) {
    throw new Error(
      `${build_vars_file} exists, ERROR env is different: ${env} <> ${data.env}`
    );
  }
  if (data.branch !== branch) {
    throw new Error(
      `${build_vars_file} exists, WARNING branch is different: ${branch} <> ${data.branch}`
    );
  }
};
...
module.exports.versionClick = (version_level) => {
  ...
  if (version_level_index === -1) {
    throw new Error(
      `version_level MUST be one of: major, minor, OR patch; not ${version_level}`
    );
  }
  const package_data = module.exports.getJSONFileData("package.json");
  const version_split =
    package_data.version && /^(\d+)\.(\d+)\.(\d+)$/.exec(package_data.version);
  if (!version_split || version_split.length < 4) {
    throw new Error(
      `missing or invalid version number in package.json, MUST be n.n.n, not ${package_data.version}`
    );
  }
  const version_new = [
  ...
  module.exports.gitTagAndPush(version_new_str);
  if (
    package_data.ultimake_settings &&
    package_data.ultimake_settings.exec_on_version_click
  ) {
